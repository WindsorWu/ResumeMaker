...
export const PreviewPageFooter = () => {
  const { isSafari, isChrome, isFirefox, isMobile } = useBrowserDetection();
  ...
  const getFooterTip = () => {
    ...
    if (isSafari) {
      return 'Safari - 建议使用浏览器的"打印"功能，选择"另存为PDF"';
    }
    ...
    if (isFirefox) {
      return 'Firefox - 建议使用浏览器的"打印"功能，选择"保存为PDF"';
    }

    return '建议使用浏览器的"打印"功能导出为 PDF';
  };
